...
export default function DashboardClient({ shownProblems, queueCount, recentProblems, dailyCommitment }: Props) {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white px-4 py-8 max-w-2xl mx-auto">
      ...
      <section className="mb-8">
        ...
        {shownProblems.length === 0 ? (
          <div className="bg-zinc-900 rounded-xl p-6 text-center">
            <p className="text-zinc-400 text-sm">Nothing due today. Add a problem or come back tomorrow.</p>
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            {shownProblems.map(p => (
              <div key={p.id} className="bg-zinc-900 rounded-xl p-4 flex items-center justify-between">
                <div>
                  ...
                  <div className="flex gap-3 mt-1">
                    ...
                    <span className="text-zinc-500 text-xs">{daysOverdue(p.next_review_date)}</span>
                    <span className="text-zinc-500 text-xs">Reviewed {p.review_count}x</span>
                  </div>
                </div>
                <button
      ...
      {/* Recently added */}
      <section>
        <h2 className="text-zinc-400 text-sm font-medium mb-3 uppercase tracking-wide">Recently Added</h2>
        {recentProblems.length === 0 ? (
          ...
          <div className="flex flex-col gap-2">
            {recentProblems.map(p => (
              <div key={p.id} className="bg-zinc-900 rounded-xl px-4 py-3 flex items-center justify-between">
